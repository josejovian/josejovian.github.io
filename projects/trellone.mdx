...
techs: ["next", "express", "mongo", "node", "redux", "javascript"]
...
This feature is also one of the most complex features in this application. Along with the previous feature, I actually spent a while to understand the Drag and Drop library, then using the library to implement drag and drop cards within the same list or between lists.  

One issue that I quickly noticed, as soon as I completed a basic implemenation of the library in this project, is that the feature does not work as smoothly during the time window, where the cursor is dragging and overlapping the card. Initially, I could not figure out the appropriate method to make it such that when a card is being clicked from any position within the card, then dragged, the mouse's position does not overlap the card.

Soon, I came up with a quick solution; **I added a small triple dot icon on the bottom of the card**, to **indirectly encourage** the users to think that, in order to drag a card, they must first click the triple dot icon, which can be seen in this project's thumbnail. However, beforehand, I also intentionally made it such that when the card is clicked from this icon, then dragged, the card will slightly move upwards from the cursor. Therefore, **allowing the cursor to not overlap with the card**.